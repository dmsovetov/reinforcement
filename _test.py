...
##%% dqn
class DQN():
    def __init__(self, n_states, n_actions, batch_size=64, lr=1e-4, gamma=0.99, mem_size=int(1e5), learn_step=5,
                 tau=1e-3):
    ...
    def learn(self, experiences):
        ...
        state_action_values = self.net_target(states).gather(1, actions)

        with torch.no_grad():
            next_state_values = self.net_eval(next_states).max(1)[0].unsqueeze(-1)
...
def train(env, agent, n_episodes=2000, max_steps=1000, eps_start=1.0, eps_end=0.1, eps_decay=0.995, target=200,
    ...
    pbar = trange(n_episodes, unit="ep", bar_format=bar_format)
    for idx_epi in pbar:
        state, _ = env.reset()
        score = 0
        for idx_step in range(max_steps):
            ...
            agent.save2memory(state, action, reward, next_state, done or trunc)
            state = next_state
            score += reward
